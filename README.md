...
This is an initial implementation targeting CNPG-I 0.5.0. Local integration tests passed against a real three-instance CNPG cluster, including switchover, standby replacement, and automatic failover while discovery was stopped in annotation mode. See the [validation record](docs/validation.md) for versions, coverage, and deployment checks still required in your environment. Images and charts must currently be built/installed from this checkout.
...
- `kubectl` configured for the intended cluster, Helm 3+, and Docker/buildx to build the image. Go is needed only for local binary development.
- A registry reachable by the Kubernetes nodes, including image-pull credentials when required.
- cert-manager, or the three existing TLS Secrets described under [certificate options](#certificate-options).
- For the verification commands: `openssl`, `grpcurl`, and `jq`.
...
| CNPG Cluster | `app-db` |
| Image | `registry.example.com/cnpg-connect-plugin:0.1.0-dev` |
...
Run the commands from this repository's root. Keep environment-specific values and credentials outside version control; `work/` is ignored by this checkout.

### 1. Build and publish the image

Replace the registry before running:

```sh
make image IMAGE=registry.example.com/cnpg-connect-plugin:0.1.0-dev
docker push registry.example.com/cnpg-connect-plugin:0.1.0-dev
```

The image must match the nodes' CPU architecture. To publish a multi-architecture image instead:

```sh
docker buildx build \
  --platform linux/amd64,linux/arm64 \
  --tag registry.example.com/cnpg-connect-plugin:0.1.0-dev \
  --push .
```

For a local kind cluster, build for its node architecture, load the image with `kind load docker-image IMAGE --name CLUSTER`, and retain `image.pullPolicy: IfNotPresent`. A registry push is unnecessary in that case.
...
Create `work/values.yaml`, replacing the registry:

```yaml
fullnameOverride: cnpg-connect
replicaCount: 1
image:
  repository: registry.example.com/cnpg-connect-plugin
  tag: "0.1.0-dev"
...
The defaults generate a private CA and three leaf certificates with cert-manager, and expose discovery only inside Kubernetes. If the registry is private, add `imagePullSecrets: [{name: registry-credentials}]` and provision that Secret in `cnpg-system`. Certificate alternatives and external access are covered below.

### 4. Install and wait for readiness

```sh
helm lint ./chart -f work/values.yaml
...
```sh
kubectl apply -f examples/cluster-annotation.yaml
...
`-authority` preserves the certificate's Service DNS identity while dialing localhost. gRPC reflection is not enabled; provide the checked-in `.proto` as shown. No client certificate is required on the application listener.
...
| `image.repository`, `image.tag` | `cnpg-connect-plugin`, `0.1.0-dev` | Image built and published by you |
...
Build and publish a new image under a new immutable tag, update `image.tag` in your values file, and review changes to the chart and API contract. Keep all active overlays in the upgrade command:

```sh
helm upgrade connect ./chart \
  --namespace cnpg-system \
  -f work/values.yaml \
  --wait --timeout 5m
kubectl -n cnpg-system rollout status deployment/cnpg-connect --timeout=180s
...
For an external installation, also pass `-f work/values-external.yaml` and any certificate overlay. Omitting an overlay can change exposure or certificate settings. Recheck `GetTopology` and stream recovery after the rollout.
...
| `ImagePullBackOff` / executable format error | Registry reachability, imagePullSecrets, image tag, and node CPU architecture |
...
| gRPC reflection / service-list command fails | Reflection is not registered; provide `-import-path proto -proto cnpg/connect/v1/topology.proto` |
...
Building the binary requires Go **1.26.4+**. Docker builds use the toolchain pinned in [Dockerfile](Dockerfile).
...
After changing the protobuf contract, install Buf v1.25+ and run `make generate`. The target installs pinned Go generators under `work/bin`. CI builds the binary and image, runs Go checks and race tests, and renders chart configurations; it does not publish images/charts or deploy.
