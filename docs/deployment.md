...
The plugin maintains shared Kubernetes `LIST/WATCH` subscriptions for Clusters, CNPG Pods, and Secret metadata. Initial synchronization and watch recovery can perform lists; ordinary observation does not repeatedly list every database or reread Cluster/Pod objects. Secret watches receive metadata only, and the plugin fetches public CA material only for referenced Secrets with active demand. Relevant changes enqueue affected Clusters, with repeated events coalesced by the work queue. There is no global periodic topology scan or fixed 100 ms event debounce. Optional CNPG-I hooks enqueue the same key and return immediately without waiting for status collection.
...
Cold startup has a separate Kubernetes API budget. Each distinct CA Secret referenced by active Clusters needs an initial fetch. Current source defaults to 100 QPS / 200 burst per Kubernetes client, up from 20 / 40 in `0.0.5`. Before network latency, the rate budget alone is roughly three seconds for 500 distinct CA reads and eight seconds for 1,000. Clusters referencing the same namespaced Secret share its cached public CA. Secret identity and resource version determine freshness; changed or deleted Secrets invalidate affected observations.
...
There is no periodic CA refresh. Secret changes trigger CA reads for active consumers. Unused entries are reclaimed; returning demand can fetch an evicted CA again. Include cache misses, informer initialization/recovery, changed Secrets, failed-read retries, and all plugin replicas when choosing API limits.

Informer caches retain only the Cluster/Pod fields needed for routing and Secret names, namespaces, UIDs, and resource versions. They exclude large Pod environments, volumes, managed fields, and unrelated Cluster configuration. Cache size still scales with watched metadata; snapshots, subscriber channels, and gRPC transports add memory proportional to active Clusters and connected clients. For a large installation, an explicit resource allocation such as the following is a starting point to measure, not a capacity guarantee:
...
| Core `secrets` | `get`, `list`, `watch` | Watch Secret metadata; fetch referenced PostgreSQL server CA's public `ca.crt` |

The observer fetches the Secret named by `status.certificates.serverCASecret`, extracts public certificate PEM blocks from `ca.crt`, and publishes them as connection metadata. It never publishes private keys or PostgreSQL passwords. The Secret informer requests metadata-only responses, but Kubernetes RBAC does not restrict `list/watch` permission to metadata or `get` permission to a single data key. Namespace-scoped deployment limits metadata and Secret access. No Pod proxy permission is needed. When upgrading an installation with manually managed RBAC, add Secret `list/watch` permissions; Helm upgrades update the chart's Role or ClusterRole automatically.
...
Secret volumes use directory mounts without `subPath`. TLS files are reloaded for new handshakes after Kubernetes projects updates; existing connections retain their TLS session. The discovery listener closes connections after five minutes and 30 seconds so clients reconnect with current trust. Its HTTP/2 admission transport does not send an early age-based GOAWAY; a client must resubscribe after closure. The CNPG-I listener retains native gRPC age-based draining. Gateway certificates follow that Gateway's rotation mechanism. Private discovery root replacement requires updating client trust; publicly trusted renewals use normal system trust.

PostgreSQL public CA material is cached by Secret identity and resource version. A Secret update or deletion invalidates affected observations, including rotations that leave Cluster certificate metadata unchanged. Only active consumers cause CA material to be fetched; Clusters sharing a Secret reuse the same result. Initial Secret informer synchronization is required for readiness, and a disconnected metadata watch stops observation renewal until recovery. A changed CA changes the topology revision, and applications receive it through discovery. CNPG still manages the database's own certificate lifecycle.
